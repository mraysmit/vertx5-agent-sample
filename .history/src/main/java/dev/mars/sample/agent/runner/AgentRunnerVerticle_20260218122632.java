 ...
 *   <li>If {@code stop == false}, loops back to step 3 (up to {@link #MAX_STEPS}).</li>
 * </ol>
 *
 * <h2>Safety</h2>
 * The hard step limit ({@value #MAX_STEPS}) prevents runaway loops when the
 * LLM never sets {@code stop: true}. Only tools present in the injected
 * allow-list can be invoked — anything else is rejected with a clear error.
 *
 * @see LlmClient
 * @see Tool
 * @see MemoryStore
...
public class AgentRunnerVerticle extends AbstractVerticle {
  ...
  public void start(Promise<Void> startPromise) {
    vertx.eventBus().consumer(EventBusAddresses.AGENT_REQUIRED, msg -> {
